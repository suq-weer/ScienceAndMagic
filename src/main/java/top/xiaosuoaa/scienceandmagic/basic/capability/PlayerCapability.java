...
public class PlayerCapability {
	...
	public static float useAttackCount(Player player, int damageType, int damageValue) {
		PlayerCapability playerCapability = player.getCapability(NeoModRegister.PLAYER_CAPABILITY_HANDLER);
		if (playerCapability != null) {
			double bpATK = 0;
			switch (damageType) {
				case 0 -> bpATK = playerCapability.getBaseATK() + damageValue * playerCapability.getPhysicATK();
				case 1 -> bpATK = playerCapability.getBaseATK() + damageValue * playerCapability.getMagicATK();
				case 2 -> bpATK = playerCapability.getBaseATK() + damageValue * playerCapability.getScienceATK();
			}
			Random random = new Random();
			double isBLD = random.nextDouble();
			ScienceAndMagic.info(String.valueOf(isBLD));
			if (isBLD <= playerCapability.getBLD()) {
				bpATK += random.nextDouble(0, playerCapability.getBLDk());
				ScienceAndMagic.info(String.valueOf(bpATK));
			}
			bpATK *= playerCapability.getPE();
			ScienceAndMagic.info(String.valueOf(bpATK));
			return (float) (bpATK >= bpATK + 0.5D ? bpATK + 1 : bpATK);
		}
		return 1;
	}
